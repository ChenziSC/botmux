...
托管的本地 Codex（tmux，含 `aiden x codex`）会话卡片提供「思考强度」下拉框。仅修改当前会话；空闲时选择后保存配置并休眠 CLI，下一条消息从原生历史续接，不重发旧任务。执行中、等待输入、接管的外部 CLI 和未验证的后端会拒绝切换。卡片区分所选配置与上一轮实际强度；机器人默认值仍由各机器人配置管理，不修改本机 Codex 默认值。
